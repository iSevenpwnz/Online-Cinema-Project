...
async def get_movie_list(
    ...
    :type page: int
    :param page_size: The number of items to display per page (must be between 1 and 20).
    :type page_size: int
    ...
    if search:
        stmt = stmt.where(
            ...
            MovieModel.stars.ilike(f"%{search}%") |
            MovieModel.directors.ilike(f"%{search}%")
        )
    ...
    movie_list = [MovieListSchema.from_orm(movie) for movie in movies]
    ...
    response = MovieListPaginatedSchema(
        movies=movie_list,
        prev_page=(
            HttpUrl(f"/movies/?page={page - 1}&page_size={page_size}")
            if page > 1
            else None
        ),
        next_page=(
...
async def get_movies_by_genre(
    ...
    return MovieListPaginatedSchema(
        movies=[MovieListSchema.from_orm(movie) for movie in movies],
        prev_page=(
            HttpUrl(f"/genres/{genre_id}/movies?page={page - 1}&page_size={page_size}")
            if page > 1 else None
        ),
        next_page=(
            HttpUrl(f"/genres/{genre_id}/movies?page={page + 1}&page_size={page_size}")
...
async def create_movie(
    ...
    """
    Add a new movie to the database with automatic handling of related entities.

    Creates or links:
    - Directors (if new names are provided)
    - Genres (if new genres are provided)
    - Stars (if new actors are provided)
    - Certification (must exist)

    Args:
        movie_data: Movie data including related entities
        db: Async database session

    Returns:
        MovieDetailSchema: Created movie with full details

    Raises:
        HTTPException: 400 for invalid data, 409 for duplicate movie
    """
...
@router.get("/{star_id}", tags=["Stars"], response_model=StarDetailSchema)
...
@router.put("/{star_id}", tags=["Stars"], response_model=StarSchema)
...
@router.delete("/{star_id}", tags=["Stars"], status_code=status.HTTP_204_NO_CONTENT)
