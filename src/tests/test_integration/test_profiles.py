...
async def test_create_user_profile_with_fake_s3(
    ...
    """
        Tests successful creation of a user profile with avatar upload to fake S3 storage.
        
        Creates and activates a user, generates an access token, and sends a POST request to create a profile with an avatar image. Verifies that the profile is created in the database, the avatar is uploaded to the fake S3 storage, and the response contains correct profile data and avatar URL.
        """
...
async def test_create_user_profile_invalid_auth(client, headers, expected_status, expected_detail):
    """
    Tests that profile creation fails with a 401 Unauthorized error when the Authorization header is missing or incorrectly formatted.
...
async def test_create_user_profile_expired_token(client, jwt_manager):
    """
    Tests that creating a user profile with an expired JWT access token fails with a 401 Unauthorized error and the appropriate error message.
    """
...
async def test_inactive_user_cannot_create_profile(
    ...
    """
        ...
        Verifies that profile creation is blocked for users who are not active, and ensures no profile is created in the database.
        """
...
async def test_profile_creation_fails_on_s3_upload_error(
    ...
    """
        Tests that user profile creation fails with a 500 error if avatar upload to S3 storage fails.
...
async def test_profile_creation_invalid_name(client, jwt_manager, first_name, last_name, expected_error):
    """
    Tests that profile creation fails when first or last name contains invalid characters.
    
    Sends a profile creation request with a first or last name containing non-English letters or digits, and asserts that the response status is 422 Unprocessable Entity with the expected error message.
    """
...
async def test_profile_creation_invalid_avatar_format(client, jwt_manager):
    ...
    Sends a profile creation request with a GIF avatar image and expects a 422 Unprocessable Entity response with an error message indicating an invalid image format.
    """
    access_token = jwt_manager.create_access_token({"user_id": 1})
...
async def test_profile_creation_avatar_too_large(db_session, client, jwt_manager):
    """
    Tests that profile creation fails when the avatar image exceeds the 1MB size limit.
    
    Attempts to create a user profile with an in-memory JPEG image larger than 1MB as the avatar. Expects a 422 Unprocessable Entity response with an error message indicating the image size exceeds the allowed limit.
    """
    access_token = jwt_manager.create_access_token({"user_id": 1})
...
async def test_profile_creation_invalid_gender(client, jwt_manager):
    ...
    Sends a profile creation request with a gender value not among the allowed options and verifies that the response contains an appropriate validation error message.
    """
    access_token = jwt_manager.create_access_token({"user_id": 1})
...
async def test_profile_creation_invalid_birth_date(client, jwt_manager, birth_date, expected_error):
    ...
    Sends a profile creation request using a birth date that is either too old or indicates the user is underage, and asserts that the response contains the expected validation error message.
    """
    access_token = jwt_manager.create_access_token({"user_id": 1})
    profile_url = "/api/v1/profiles/users/1/profile/"
    headers = {"Authorization": f"Bearer {access_token}"}
...
async def test_profile_creation_empty_info(client, jwt_manager, info_value):
    """
    Tests that profile creation fails when the info field is empty or contains only whitespace.
    
    Sends a profile creation request with an invalid info value and asserts a 422 Unprocessable Entity response with an error message indicating the info field cannot be empty or spaces only.
    """
    access_token = jwt_manager.create_access_token({"user_id": 1})
    profile_url = "/api/v1/profiles/users/1/profile/"
    headers = {"Authorization": f"Bearer {access_token}"}
