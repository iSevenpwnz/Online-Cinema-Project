...
@pytest.mark.asyncio
async def test_create_order_from_cart_success(client: AsyncClient,
                                              auth_headers: dict,
...
@pytest.mark.asyncio
async def test_get_order_history_no_filter(client: AsyncClient,
                                           auth_headers: dict,
                                           db_session: AsyncSession,
                                           user: UserModel,
                                           movie: MovieModel):
...
@pytest.mark.asyncio
async def test_get_order_history_with_filter(client: AsyncClient,
                                             auth_headers: dict,
                                             db_session: AsyncSession,
                                             user: UserModel,
                                             movie: MovieModel):
...
@pytest.mark.asyncio
async def test_get_order_by_id_success(client: AsyncClient,
                                       auth_headers: dict,
                                       db_session: AsyncSession,
                                       user: UserModel,
                                       movie: MovieModel):
...
@pytest.mark.asyncio
async def test_change_order_status_success(client: AsyncClient,
                                           auth_headers: dict,
                                           db_session: AsyncSession,
                                           user: UserModel,
                                           movie: MovieModel):
    ...
    assert response.status_code == status.HTTP_200_OK
    assert response.json()["status"] == OrderStatusEnum.PAID.value
