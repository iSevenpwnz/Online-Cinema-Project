class BaseS3Error(Exception):
    ...
    def __init__(self, message=None):
        """
        Initializes the BaseS3Error with an optional custom error message.
        
        Args:
            message: Optional custom error message. Defaults to a generic S3 storage error message if not provided.
        """
        if message is None:
...
class S3ConnectionError(BaseS3Error):
    ...
    def __init__(self, message="Failed to connect to S3 storage."):
        """
        Initializes an S3ConnectionError with a default or custom error message.
        
        Args:
            message: Optional custom error message describing the connection failure.
        """
        super().__init__(message)
...
class S3FileNotFoundError(BaseS3Error):
    ...
    def __init__(self, message="Requested file not found in S3."):
        """
        Initializes the exception for a missing file in S3 storage.
        
        Args:
            message: Optional custom error message describing the missing file scenario.
        """
        super().__init__(message)
...
class S3PermissionError(BaseS3Error):
    ...
    def __init__(self, message="Insufficient permissions to access S3 resource."):
        """
        Initializes the exception for insufficient permissions when accessing an S3 resource.
